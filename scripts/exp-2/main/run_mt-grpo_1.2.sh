...
if [ $# -gt 0 ]; then
  ...
  STAMP=$(date +%F_%H-%M-%S)
  exp_name="MT-GRPO_1.2_EXP_2_${STAMP}"
fi
...
python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.swap_space=16 \
    ...
    trainer.sec.bandit.lambda_bias=1.2 \
    trainer.sec.bandit.lambda_bias_type='add' \
    actor_rollout_ref.actor.clip_ratio_high=0.28 \
    trainer.rejection_sample=True \
    trainer.rejection_sample_multiplier=3.0 \
    trainer.save_freq=180 \
    trainer.pure_acc_filter=False \
