...
# ---- device helper ---------------------------------------------------------
try:
    from verl.utils.device import is_cuda_available as _is_cuda_available
except Exception:
...
try:
    from verl.utils.fs import (
        copy_to_local as _copy_to_local,
        is_non_local as _is_non_local,
        local_mkdir_safe as _local_mkdir_safe,
    )
except Exception:
    # Older verl might only have HDFS helpers. Minimal local-only shims:
    def _copy_to_local(path, *args, **kwargs):
        # assume path is already local
        return path

    def _is_non_local(path):
        return False

    def _local_mkdir_safe(path):
        os.makedirs(path, exist_ok=True)
        return path

copy_to_local = _copy_to_local
is_non_local = _is_non_local
local_mkdir_safe = _local_mkdir_safe


# ---- FSDP utils ------------------------------------------------------------
try:
    from verl.utils.fsdp_utils import (
        fsdp_version as _fsdp_version,
        get_fsdp_full_state_dict as _get_fsdp_full_state_dict,
        get_fsdp_state_ctx as _get_fsdp_state_ctx,
    )
except Exception:
    # Minimal implementation for classic FSDP v1 only
    from contextlib import nullcontext
    from torch.distributed.fsdp import FullStateDictConfig

    def _fsdp_version(model):
        # Treat classic FSDP as version 1
        return 1 if isinstance(model, FSDP) else 0

    def _get_fsdp_state_ctx(model, state_type, state_cfg, optim_cfg):
        if _fsdp_version(model) == 1:
            return FSDP.state_dict_type(model, state_type, state_cfg, optim_cfg)
        else:
            return nullcontext()

    def _get_fsdp_full_state_dict(
...
fsdp_version = _fsdp_version
get_fsdp_state_ctx = _get_fsdp_state_ctx
get_fsdp_full_state_dict = _get_fsdp_full_state_dict


# ---- logging helper --------------------------------------------------------
try:
    from verl.utils.logger import log_with_rank as _log_with_rank
except Exception:
    def _log_with_rank(msg, rank=None, logger=None, log_only_rank_0=False):
        if log_only_rank_0 and rank not in (0, None):
            return
        print(f"[rank {rank}] {msg}")

log_with_rank = _log_with_rank
