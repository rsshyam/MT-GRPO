...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        import os
        reserve_gb = float(os.getenv("VRAM_RESERVE_GB", "0"))
        if reserve_gb > 0:
            torch.cuda.synchronize()
            bytes_to_hold = int(reserve_gb * (1024**3))
            # keep a reference on the instance so it stays alive
            self._cuda_reserve = torch.empty(bytes_to_hold, dtype=torch.uint8, device="cuda")
    ...
    def generate_sequences(self, prompts: DataProto, max_retries: int = 1e9, **kwargs) -> DataProto:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Generate sequences
                # after you compute do_sample and kwargs
                ...
                if (self.config.n > 1  and do_sample) or ('n' in kwargs and kwargs['n'] > 1 and do_sample):
                    if 'n' in kwargs:
                        current_n = kwargs['n']
                    else:
                        current_n = self.config.n
