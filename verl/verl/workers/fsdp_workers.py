...
from verl.utils.checkpoint import FSDPCheckpointManager
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_actor_checkpoint_manager(self):
        """
        Build FSDPCheckpointManager for the actor.
        Must be called AFTER self.actor_module_fsdp, self.actor_optimizer,
        self.actor_lr_scheduler, and self.tokenizer are created.
        """
        ...
        ckpt_cfg = DictConfig(
            {
                ...
                # what to save
                # include "hf_model" if you still want a full HF model for easy inference
                "save_contents": ["model", "optimizer", "extra"],
            }
        )

        self.actor_ckpt_manager = FSDPCheckpointManager(
            model=self.actor_module_fsdp,           # FSDP-wrapped actor
            optimizer=self.actor_optimizer,         # AdamW for the actor
            lr_scheduler=self.actor_lr_scheduler,   # LR scheduler
            processing_class=self.tokenizer,        # tokenizer / processor
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            # build actor checkpoint manager
            if self._is_actor and self.config.get('use_checkpoint_manager', False):
                self._build_actor_checkpoint_manager()

                # auto-resume only if the checkpoint manager is enabled
                resume_path = self.config.get("resume_from", None)
                if resume_path is not None:
                    print(f"[rank {self.rank}] init_model: loading actor checkpoint from {resume_path}")
                    self.actor_ckpt_manager.load_checkpoint(
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # Pull per-batch sampling overrides (e.g., n, temperature, top_p, etc.)
        sampling_overrides = prompts.meta_info.pop("sampling_overrides", {})
        with self.rollout_sharding_manager:
            log_gpu_memory_usage('After entering rollout sharding manager', logger=logger)

            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_sequences(prompts=prompts, **sampling_overrides)
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        if not self.config.get('use_checkpoint_manager', False):
            assert self._is_actor
            import torch
            if self._is_offload_param:
                load_fsdp_param_and_grad(module=self.actor_module_fsdp,
            ...
            # Ensure the manager exists (in case init_model wasn't called for some reason)
            if not hasattr(self, "actor_ckpt_manager"):
                self._build_actor_checkpoint_manager()

            # Bring params to GPU if you're using manual offload
            if self._is_offload_param:
                load_fsdp_param_and_grad(
            ...
            # Use some step number for naming/rotation. Adjust as needed for your setup.
            # If you track step in the actor, you could do: getattr(self.actor, "global_step", 0)
            global_step = int(getattr(self, "global_step", 0))

            # Optional: max number of checkpoints to keep
            max_to_keep = getattr(self, "max_actor_ckpt_to_keep", None)

            print(f"[rank {self.rank}] Saving actor checkpoint to {local_path} (global_step={global_step})")

            self.actor_ckpt_manager.save_checkpoint(
                local_path=local_path,
                hdfs_path=hdfs_path,        # manager currently ignores this, but OK to pass
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        ...
        # Old path (HF-only) doesn’t really support true resume, so only support the new manager path
        if not self.config.get('use_checkpoint_manager', False):
            raise RuntimeError(
                "load_checkpoint is only supported when config.use_checkpoint_manager=True "
                "(old checkpoints only saved HF weights, not optimizer/scheduler)."
            )

        if not hasattr(self, "actor_ckpt_manager"):
            self._build_actor_checkpoint_manager()

        print(f"[rank {self.rank}] Loading actor checkpoint from {local_path}")
